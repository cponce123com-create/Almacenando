...
// Resets ALL auto-matched EXACT products back to NONE so the matcher can run
// fresh. Products manually linked (msdsMatchedBy = 'manual') are preserved
// unless resetManual=true is explicitly passed.
// After resetting, immediately runs a full rescan against Drive.
...
router.post("/reset-all", requireAuth, requireRole("admin", "supervisor"), asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  for (const p of allProducts) {
    // Skip manually linked products unless resetManual=true
    if (!resetManual && p.msdsMatchedBy === "manual") continue;
    // Only reset products that have some match data (skip already-clean ones)
    if (!p.msdsStatus || p.msdsStatus === "NONE") continue;
  ...
  for (const product of products) {
    // Respect manually linked products
