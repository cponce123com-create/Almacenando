...
router.put("/", requireAuth, async (req, res) => {
  const userId = (req as typeof req & { userId: string }).userId;
  const { burialType, ceremonyType, musicNotes, dressCode, guestNotes, locationNotes, additionalNotes } = req.body;

  const existing = await db.select().from(funeralPreferencesTable).where(eq(funeralPreferencesTable.userId, userId)).limit(1);
  if (existing.length === 0) {
    const id = generateId();
    await db.insert(funeralPreferencesTable).values({ id, userId, burialType, ceremonyType, musicNotes, dressCode, guestNotes, locationNotes, additionalNotes });
    const created = await db.select().from(funeralPreferencesTable).where(eq(funeralPreferencesTable.id, id)).limit(1);
    res.json(toPrefs(created[0]!));
    return;
  }
