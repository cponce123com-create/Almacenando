...
// Resets ALL auto-matched products back to NONE, then immediately re-scans.
// Manual links (msdsMatchedBy = 'manual') are preserved unless resetManual=true.
...
router.post("/reset-all", requireAuth, requireRole("admin", "supervisor"), asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  const allProducts = await db.select().from(productsTable).where(warehouseCondition);
  const now = new Date();
  let resetCount = 0;
