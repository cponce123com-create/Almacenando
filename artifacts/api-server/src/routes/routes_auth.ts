...
router.post("/login", authLoginLimiter, asyncHandler(async (req, res) => {
  ...
  // Protección contra enumeración por timing: si el usuario no existe,
  // hacemos un bcrypt.compare dummy para igualar el tiempo de respuesta.
  if (users.length === 0) {
  ...
  if (user.status !== "active") {
    // También consumimos tiempo aquí para uniformidad.
    await dummyCompare(password);
  ...
  // Cookie httpOnly + Secure + SameSite=Strict — no accesible desde JS.
  ...
  // El token YA NO se devuelve en el body — la cookie lo gestiona todo.
  // Mantenemos el shape `{ user }` para compatibilidad con el frontend.
  res.json({
    user: {
      id: user.id,
      email: user.email,
...
router.post("/setup", asyncHandler(async (req, res) => {
  ...
  // Validar que la contraseña cumpla la política antes de crear el admin.
  const passwordCheck = passwordSchema.safeParse(password);
  if (!passwordCheck.success) {
    res.status(400).json({
      error: "ADMIN_PASSWORD no cumple la política de seguridad: " + (passwordCheck.error.issues[0]?.message ?? "inválida"),
    });
    return;
  }
