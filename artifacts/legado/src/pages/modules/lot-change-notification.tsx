...
export default function LotChangeNotificationPage() {
  ...
  const { data: allProducts = [], isLoading: loadingProducts } = useQuery<Product[]>({
    ...
    queryFn: () =>
      api("/api/products?limit=2000&status=active").then((r: any) => r.data ?? r),
  });
  ...
  const validate = (): boolean => {
    ...
    if (!form.productionOrder.trim())
      errs.productionOrder = "La orden de producción es requerida";
    setErrors(errs);
  ...
  const sendMutation = useMutation({
    mutationFn: () =>
      api("/api/notifications/lot-change", {
        method: "POST",
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-100 p-6 space-y-5">
              ...
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={`w-full justify-between font-normal ${
                    errors.productId ? "border-red-400 focus:ring-red-400" : ""
                  }`}
                >
                  {selectedProduct ? (
                    ...
                    <span className="text-slate-400">
                      {loadingProducts ? "Cargando productos…" : "Buscar producto…"}
                    </span>
                  )}
          ...
          <Button
            ...
            className="w-full text-white gap-2 h-11 text-base font-semibold bg-amber-500 hover:bg-amber-600 transition-colors duration-200"
          >
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
          ...
          <p className="text-xs text-slate-400 mt-3">
            El correo se enviará a todos los destinatarios con el asunto:
            <br />
            <span className="font-medium text-slate-500 italic">
              "Notificación de Cambio de Lote — [nombre del producto]"
            </span>
          </p>
        </div>
      </div>
